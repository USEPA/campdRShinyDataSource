library(sf)
# R script for writing up shapefile data for leaflet
...
countyStateSf <- get_county_search_data("R/shapefiles/cb_2018_us_county_5m.shp","R/shapefiles/stateFIPS.csv")
countyStateSf <- countyStateSf[with(countyStateSf, order(stateName, countyName)), ]
stateSf <- get_state_search_data("R/shapefiles/cb_2018_us_state_5m.shp")

st_write(countyStateSf, dsn = "data/facility-map/shapefiles/countyStateSf.shp")
st_write(stateSf, dsn = "data/facility-map/shapefiles/stateSf.shp")
